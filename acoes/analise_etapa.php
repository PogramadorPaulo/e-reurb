...
try {
    // Verificar se a etapa já está em análise
    $stmt = $db->prepare("
    ...
    $etapaName = "Etapa {$etapa} do processo: {$numero_procedimento}, Núcleo: {$nucleo_nome} está Em Análise.";
    ...
    $historico->bindValue(':h_statusID', 5);
...
");
    ...
    if (!empty($users)) {
        ...
        $notificationContent = "
            ...
            está em <strong>'Em Análise'</strong>.<br>
            {$justificativa}
        ";
        ...
        foreach ($users as $user) {
            ...
            $notification->bindValue(':not_tipo', 'Etapa em Análise');
