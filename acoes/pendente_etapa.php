...
$justificativa = filter_input(INPUT_POST, "descricao", FILTER_SANITIZE_SPECIAL_CHARS);

if (!hasPermission($id_user, 'processo_etapaPendente', $db)) {
    echo json_encode([
        'status' => 'error',
        'tittle' => 'Erro',
...
try {
    ...
    if ($etapaStatus && $etapaStatus['procedimento_status'] == 1) {
        echo json_encode([
            'status' => 'warning',
            'tittle' => 'Atenção',
            'message' => 'Esta etapa já está pendente.',
            'icon' => 'warning'
        ]);
        exit;
    }
    ...
    $etapaName = "Etapa {$etapa} do processo: {$id_procedimento}, Núcleo: {$nucleo_nome} está Pendente.";
    ...
    $historico->bindValue(':h_statusID', 1);
...
");
    ...
    if (!empty($users)) {
        ...
        $notificationContent = "
            ...
            está <strong>'Pendente'</strong>.<br>
            {$justificativa}
        ";
        ...
        foreach ($users as $user) {
            ...
            $notification->bindValue(':not_tipo', 'Etapa Pendente');
    ...
    echo json_encode([
        'status' => 'error',
        'tittle' => 'Erro',
        'message' => 'Erro ao colocar em pedencia a etapa: ' . $e->getMessage(),
        'icon' => 'error'
    ]);
}
