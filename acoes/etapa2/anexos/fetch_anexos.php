...
$idEtapa = 2; // Alterado para etapa 2
...
if ($total_data > 0) {
  $canDelete = hasPermission($_SESSION['uid'], 'processo_2etapa_excluir_documento', $db);
  ...
  foreach ($result as $row) {
    ...
    if ($canDelete) {
      $output .= '
                              <button class="btn delete-document-etapa2" id="delete-document" 
                    title="Excluir" 
...
<script>
$(function () {
  $('#etapa2_anexos [data-toggle="tooltip"]').tooltip({ html: true, placement: 'top' });
});
</script>