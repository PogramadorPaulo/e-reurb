...
function carregarAnexosEpata2(page = 1, id, query = '') {
    $.ajax({
        url: '../../acoes/etapa2/anexos/fetch_anexos.php',
        method: 'POST',
        data: { page, id, query },
        success: function (data) {
            $('#etapa2_anexos').html(data); // Atualiza o conteúdo dos anexos
        },
        error: function () {
            $('#etapa2_anexos').html('<p>Erro ao carregar anexos.</p>'); // Exibe erro
        }
    });
}
...
$(document).on('click', '.page-link', function () {
    ...
    const id = $('#id').val();
    const query = $('#etapa2_search_query').val(); // Pega valor de pesquisa (se houver)
    carregarAnexosEpata2(page, id, query); // Recarrega anexos com a nova página
});

// Evento de pesquisa dinâmica no input de pesquisa
$('#etapa2_search_query').on('input', function () {
    const query = $(this).val().trim(); // Pega o valor digitado
    const id = $('#id').val();
    carregarAnexosEpata2(1, id, query); // Recarrega anexos na página 1 com a pesquisa
});
...
document.addEventListener('DOMContentLoaded', function () {
    const dropzone = document.getElementById('dropzoneEtapa2');
    const fileInput = document.getElementById('arquivo_anexoEtapa2');
    const fileInfoAnexo = document.getElementById('fileInfoAnexoEtapa2');
    ...
    function handleFiles() {
        ...
        if (file) {
            ...
            document.getElementById('botaoUploadEtapa2').style.display = 'inline-block';
        }
    }
});
...
$('#arquivo_anexoEtapa2').on('change', function () {
    // Verifica se há arquivos selecionados
    $('#botaoUploadEtapa2').show(); // Exibe o botão de upload

});

// Evento de clique no botão de upload
$('#botaoUploadEtapa2').on('click', function () {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) realizarUploadEtapa2(result.value); // Realiza upload se confirmado
        else cancelarUploadEtapa2(); // Cancela a operação
    });
});

// Função para realizar o upload do arquivo via AJAX
function realizarUploadEtapa2(titulo) {
    spinner.show(); // Exibe o loader

    let formData = new FormData();
    formData.append('arquivo', $('#arquivo_anexoEtapa2')[0].files[0]); // Adiciona o arquivo
    ...
    $.ajax({
        url: '../../acoes/etapa2/anexos/upload_arquivo.php',
        data: formData,
        processData: false,
        contentType: false,
        type: 'POST',
        dataType: 'json',
        beforeSend: () => ajustarEstadoEtapa2(true), // Ajusta o estado antes de enviar
        success: (response) => tratarRespostaEtapa2(response), // Trata a resposta do servidor
        error: () => exibirMensagemEtapa2('Erro', 'Tente novamente!', 'error'), // Exibe erro
        complete: () => ajustarEstadoEtapa2(false) // Ajusta estado após completar
    });
}

// Função para tratar a resposta do servidor após o upload (JSON: status, title, message, icon)
function tratarRespostaEtapa2(response) {
    const r = typeof response === 'object' && response !== null ? response : null;
    if (!r || !r.status) {
        exibirMensagemEtapa2('Atenção', typeof response === 'string' ? response : 'Resposta inválida do servidor.', 'warning');
        return;
    }
    const icon = r.icon || (r.status === 'success' ? 'success' : 'warning');
    if (r.status === 'success') {
        atualizarListaArquivosEtapa2();
        $('#arquivo_anexoEtapa2').val('');
        $('#fileInfoAnexoEtapa2').text('');
        $('#botaoUploadEtapa2').hide();
    }
    exibirMensagemEtapa2(r.title || 'Atenção', r.message || '', icon);
}
// Função para recarregar a lista de anexos após upload
function atualizarListaArquivosEtapa2() {
    const id = $('#id').val();
    carregarAnexosEpata2(1, id); // Carrega a primeira página novamente
}

// Função para ajustar o estado da interface durante operações
function ajustarEstadoEtapa2(desativar) {
    $('#content').css('opacity', desativar ? '.5' : ''); // Ajusta opacidade
    $('#arquivo_anexoEtapa2').prop('disabled', desativar); // Habilita/desabilita input
    $('#botaoUploadEtapa2').toggle(!desativar); // Exibe/esconde botão de upload
    if (!desativar) spinner.hide(); // Esconde loader ao concluir
}

// Função para exibir mensagens usando SweetAlert
function exibirMensagemEtapa2(titulo, texto, tipo) {
    Swal.fire({ title: titulo, text: texto, icon: tipo });
}

// Função para cancelar o upload e exibir modal de cancelamento
function cancelarUploadEtapa2() {
...
$(document).on('click', '.delete-document-etapa2', function () {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.ajax({
                url: '../../acoes/etapa2/anexos/del_arquivo.php',
                method: 'POST',
                ...
                success: function (response) {
                    ...
                    if (response.status === 'success') {
                        ...
                        atualizarListaArquivosEtapa2(); // Atualiza a lista de arquivos
                    } else {
...
$(document).ready(function () {
    $('#btnGerarZipEtapa2').on('click', function () {
        ...
        $.ajax({
            url: '../../acoes/etapa2/anexos/gerar_zip.php',
            type: 'GET',
            data: {
            ...
            success: function (response) {
                ...
                if (response instanceof Blob) {
                    ...
                    a.href = url;
                    a.download = 'processo_etapa2_' + processoId + '.zip'; // Nome do arquivo para download
