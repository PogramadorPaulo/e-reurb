...
$query = "SELECT * FROM confrontantes WHERE status_con = 1";

// Filtro por ID de procedimento
if (!empty($_POST['id'])) {
  $query .= " AND id_procedimento_con = :id";
} else {
...
if ($total_data > 0) {
    ...
    $output .= '
            <div class="card p-2">
                <div class="row">
                    ...
                    <div class="col-md-3">
                        <button id="' . $row['id_con'] . '" type="submit" class="btn btn-outline-info btn-sm view_data_confrontante">
                            <i class="fa fa-pencil-square-o" aria-hidden="true"></i> Editar
                        </button>
                        <button class="btn btn-outline-danger btn-sm" onclick="deletarConfrontante(' . $row['id_con'] . ',0)">
  ...
  $output .= '<div><label class="text-secondary card p-2">Nenhum confrontante encontrado</label></div>';
}
