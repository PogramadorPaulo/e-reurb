...
$check = $db->prepare("
    SELECT id_procedimento_con, cpf, cnpj 
    FROM confrontantes 
    WHERE (cpf = :cpf OR cnpj = :cnpj) 
      AND id_procedimento_con = :id 
      AND status_con = 1 
    LIMIT 1
");
...
if ($check->rowCount() > 0) {
    echo json_encode([
        'status' => 'error',
        'title' => 'Erro',
        'message' => 'Já existe um confrontante cadastrado com este CPF ou CNPJ.',
        'icon' => 'error',
    ]);
    exit;
}
...
$sql = $db->prepare(
	...
	(id_procedimento_con, con_identificado, tipo, cpf, cnpj, i_estadual, i_municipal, representante, cargo, nome, data_nasc, sexo, rg, emissor, profissao,
	 ...
	 values(:id_procedimento, :con_identificado, :tipo, :cpf, :cnpj, :i_estadual, :i_municipal, :representante, :cargo, :nome, :data_nasc, :sexo, :rg, :emissor, :profissao,
	  :estado_civil, :uniao_estavel,  :telefone, :celular, :email, :cep, :logradouro, :numero, :complemento, :bairro, :cidade, :estado, :pai, :mae, :data_cadastro)"
);
$sql->bindValue(":id_procedimento", $id);
$sql->bindValue(":con_identificado", $identificacao);
...
if ($sql->rowCount() > 0) {
    ...
    $atividade->bindValue(":atividade_name", 'Novo confrontante cadastrado: ' . $nome);
    $atividade->execute();
