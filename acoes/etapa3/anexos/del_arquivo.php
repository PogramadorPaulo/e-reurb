...
if (isset($_POST['id']) && !empty($_POST['id'])) {
    ...
    if (isEtapaConcluida($_POST['idProcesso'], 3, $db)) {
        echo json_encode([
            'status' => 'error',
    ...
    if (count($array) > 0) {
        ...
        if (file_exists($_FileName)) {
            ...
            $deletar = $db->prepare("UPDATE tb_proprietarios_anexos SET anexo_status = :status WHERE anexo_id = :id");
