...
$tipo_pessoa =  filter_input(INPUT_POST, "tipo", FILTER_SANITIZE_SPECIAL_CHARS);
...
$check = $db->prepare("
    SELECT id_tab_procedimento, cpf, cnpj, status_tab
    FROM proprietarios_tabulares 
    WHERE (cpf = :cpf OR cnpj = :cnpj) 
      AND id_tab_procedimento = :id 
      AND status_tab = 1 
    LIMIT 1
");
...
$sql = $db->prepare(
	...
	(id_tab_procedimento, tab_identificado, tipo_pessoa, cpf, cnpj, i_estadual, i_municipal, representante, cargo, nome, data_nasc, sexo, rg, emissor, profissao,
	 ...
	 values(:id_procedimento, :tab_identificado, :tipo_pessoa, :cpf, :cnpj, :i_estadual, :i_municipal, :representante, :cargo, :nome, :data_nasc, :sexo, :rg, :emissor, :profissao,
	  :estado_civil, :uniao_estavel,  :telefone, :celular, :email, :cep, :logradouro, :numero, :complemento, :bairro, :cidade, :estado, :pai, :mae, :data_cadastro)"
);
...
if ($sql->rowCount() > 0) {
    ...
    $atividade->bindValue(":atividade_name", 'Novo proprietário cadastrado: ' . $nome);
    $atividade->execute();
