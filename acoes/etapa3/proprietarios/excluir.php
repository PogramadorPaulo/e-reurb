...
$sql = $db->prepare("SELECT id_tab from proprietarios_tabulares WHERE id_tab=:id");
$sql->bindValue(":id", $id);
$sql->execute();
if ($sql->rowCount() == 0) {
    $response = array(
    ...
    $update = $db->prepare("UPDATE proprietarios_tabulares SET status_tab=:status WHERE id_tab=:id");
