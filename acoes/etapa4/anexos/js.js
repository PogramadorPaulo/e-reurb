...
function carregarAnexosEtapa4(page = 1, id, query = '') {
    $.ajax({
        url: '../../acoes/etapa4/anexos/fetch_anexos.php',
        method: 'POST',
        data: { page, id, query },
        success: function (data) {
            $('#etapa4_anexos').html(data); // Atualiza o conteúdo dos anexos
        },
        error: function () {
            $('#etapa4_anexos').html('<p>Erro ao carregar anexos.</p>'); // Exibe erro
        }
    });
}
...
$(document).on('click', '.page-link', function () {
    ...
    const id = $('#id').val();
    const query = $('#etapa4_search_query').val(); // Pega valor de pesquisa (se houver)
    carregarAnexosEtapa4(page, id, query); // Recarrega anexos com a nova página
});

// Evento de pesquisa dinâmica no input de pesquisa
$('#etapa4_search_query').on('input', function () {
    const query = $(this).val().trim(); // Pega o valor digitado
    const id = $('#id').val();
    carregarAnexosEtapa4(1, id, query); // Recarrega anexos na página 1 com a pesquisa
});



(function () {
    function loadAnexosEtapa4() {
        const id = $('#id').val();
        carregarAnexosEtapa4(1, id);
    }
    var etapaId =
        window.EtapasTabs && typeof EtapasTabs.resolveTabIdFromPane === 'function'
            ? EtapasTabs.resolveTabIdFromPane('#etapa4_anexos', '4')
            : '4';
    if (window.EtapasTabs) {
        EtapasTabs.registerInit(etapaId, loadAnexosEtapa4);
    } else {
        $(document).ready(loadAnexosEtapa4);
    }
})();

// Arrasta e solta o arquivo de upload // 
document.addEventListener('DOMContentLoaded', function () {
    const dropzone = document.getElementById('dropzoneEtapa4');
    const fileInput = document.getElementById('arquivo_anexoEtapa4');
    const fileInfoAnexo = document.getElementById('fileInfoAnexoEtapa4');
    ...
    function handleFiles() {
        ...
        if (file) {
            ...
            document.getElementById('botaoUploadEtapa4').style.display = 'inline-block';
        }
    }
});
...
$('#arquivo_anexoEtapa4').on('change', function () {
    // Verifica se há arquivos selecionados
    $('#botaoUploadEtapa4').show(); // Exibe o botão de upload

});

// Evento de clique no botão de upload
$('#botaoUploadEtapa4').on('click', function () {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) realizarUploadEtapa4(result.value); // Realiza upload se confirmado
        else cancelarUploadEtapa4(); // Cancela a operação
    });
});

// Função para realizar o upload do arquivo via AJAX
function realizarUploadEtapa4(titulo) {
    spinner.show(); // Exibe o loader

    let formData = new FormData();
    formData.append('arquivo', $('#arquivo_anexoEtapa4')[0].files[0]); // Adiciona o arquivo
    formData.append('titulo', titulo); // Adiciona o título
    formData.append('id', $('#id').val()); // Adiciona o ID
    formData.append('idUser', $('#idUser').val()); // Adiciona o ID Ussuário
    formData.append('idProcedimento', $('#idProcedimento').val()); // Adiciona o ID
    $.ajax({
        url: '../../acoes/etapa4/anexos/upload_arquivo.php',
        data: formData,
        processData: false,
        contentType: false,
        type: 'POST',
        dataType: 'json',
        beforeSend: () => ajustarEstadoEtapa4(true), // Ajusta o estado antes de enviar
        success: (response) => tratarRespostaEtapa4(response), // Trata a resposta do servidor
        error: () => exibirMensagemEtapa4('Erro', 'Tente novamente!', 'error'), // Exibe erro
        complete: () => ajustarEstadoEtapa4(false) // Ajusta estado após completar
    });
}

// Função para tratar a resposta do servidor após o upload (JSON: status, title, message, icon)
function tratarRespostaEtapa4(response) {
    const r = typeof response === 'object' && response !== null ? response : null;
    if (!r || !r.status) {
        exibirMensagemEtapa4('Atenção', typeof response === 'string' ? response : 'Resposta inválida do servidor.', 'warning');
        return;
    }
    const icon = r.icon || (r.status === 'success' ? 'success' : 'warning');
    if (r.status === 'success') {
        atualizarListaArquivosEtapa4();
        $('#arquivo_anexoEtapa4').val('');
        $('#fileInfoAnexoEtapa4').text('');
        $('#botaoUploadEtapa4').hide();
    }
    exibirMensagemEtapa4(r.title || 'Atenção', r.message || '', icon);
}
// Função para recarregar a lista de anexos após upload
function atualizarListaArquivosEtapa4() {
    const id = $('#id').val();
    carregarAnexosEtapa4(1, id); // Carrega a primeira página novamente
}

// Função para ajustar o estado da interface durante operações
function ajustarEstadoEtapa4(desativar) {
    $('#content').css('opacity', desativar ? '.5' : ''); // Ajusta opacidade
    $('#arquivo_anexoEtapa4').prop('disabled', desativar); // Habilita/desabilita input
    $('#botaoUploadEtapa4').toggle(!desativar); // Exibe/esconde botão de upload
    if (!desativar) spinner.hide(); // Esconde loader ao concluir
}

// Função para exibir mensagens usando SweetAlert
function exibirMensagemEtapa4(titulo, texto, tipo) {
    Swal.fire({ title: titulo, text: texto, icon: tipo });
}

// Função para cancelar o upload e exibir modal de cancelamento
function cancelarUploadEtapa4() {
...
$(document).on('click', '.delete-document-etapa4', function () {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            $.ajax({
                url: '../../acoes/etapa4/anexos/del_arquivo.php',
                method: 'POST',
                ...
                success: function (response) {
                    ...
                    if (response.status === 'success') {
                        ...
                        atualizarListaArquivosEtapa4(); // Atualiza a lista de arquivos
                    } else {
...
$(document).ready(function () {
    ...
    $('#btnGerarZipEtapa4').on('click', function () {
        var processoId = $('#id').val(); // Obtém o ID do processo dinamicamente
        if (!processoId) {
        ...
        $.ajax({
            url: '../../acoes/etapa4/anexos/gerar_zip.php',
            type: 'GET',
            data: {
            ...
            success: function (response) {
                ...
                if (response instanceof Blob) {
                    ...
                    a.href = url;
                    a.download = 'processo_etapa4_' + processoId + '.zip'; // Nome do arquivo para download
