...
$idEtapa = 4; // Alterado para etapa 4
...
if ($total_data > 0) {
  $canDelete = hasPermission($_SESSION['uid'], 'processo_4etapa_excluir_documento', $db);
  ...
  foreach ($result as $row) {
    ...
    if ($canDelete) {
      $output .= '
                              <button class="btn delete-document-etapa4" id="delete-document" 
                    title="Excluir" 
...
<script>
$(function () {
  $('#etapa4_anexos [data-toggle="tooltip"]').tooltip({ html: true, placement: 'top' });
});
</script>