...
$baseQuery = "FROM tb_etapas_anexos WHERE anexo_etapa = 7 AND anexo_status =1 AND anexo_prc=:anexo_prc";
...
try {
  ...
  $canEdit = hasPermission($userId, 'processso_7etapaEditarDoc', $db);
  $canDelete = hasPermission($userId, 'processso_7etapaExcluirDoc', $db);
  ...
  foreach ($result as $row) {
    ...
    if ($canDelete) {
      $output .= '
                        <button class="btn btn-light btn-sm delete-document-etapa7" id="" 
    ...
    if ($canEdit) {
      $output .= '
                    <div>
                        <button data-id="' . $doc_id . '" type="button" class="btn btn-outline-info btn-sm view_data_doc_etapa7">
